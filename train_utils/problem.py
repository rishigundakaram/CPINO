...
class NS3D(problem): 
    def __init__(self, config) -> None:
        ...
        train_loader = NSDataset(datapath1=train_config['path1'], datapath2=train_config['path2'],
        ...
        self.train_loader = train_loader.make_loader(train_loader.data.size()[0],
                                batch_size=config['train_params']['batchsize'],
                                start=train_config['offset'],
...
class Loss():
    ...
    def __call__(self, input, target, prediction):
        ...
        loss = {}
        data_loss = self.L2(target, output)
        ...
        loss["L2 loss"] = self.ic_weight * ic_loss + self.f_weight * f_loss + self.data_weight * data_loss
